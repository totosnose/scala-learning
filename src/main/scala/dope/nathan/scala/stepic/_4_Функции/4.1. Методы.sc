...
// z: => Int - передача по имени
// для сложновычисляемых выражений и содержащих side-эффекты,
// в теле метода такие параметры вычисляются только тогда, когда они действительно необходимы,
...
// то его значение можно передать блоком в фигурных скобках
...
// хвостовая рекурсия
// - если вызовы самой себя происходят только в "хвостовых точках" вычислений,
// функция м.б. оптимизирована в "хвостовую рекурсию",
// которая внутри себя будет представлять из себя цикл

// - необходимо использовать аннотацию @tailrec,
// чтобы быть уверенным, что компилятор выполнит хвостовую оптимизацию
